...
# Role
You are a writer who generates memory search queries based on key phrases from conversations.

# Guidelines
Generate specific and natural search queries.
Predict possible retrieval results that may be returned from the query.

# Examples
할머니는 2022년 3월에 허리 통증으로 병원에 가서 물리치료를 받으셨고, 현재는 통증이 완화된 상태입니다.
할머니는 작년 봄날에 공원에서 산책하시며 벚꽃 사진을 찍으셨던 기억이 있습니다.
할머니는 저번 주에 복지관 댄스 동아리에서 친구들과 춤을 추셨습니다.
할머니는 2019년에 제주도로 비행기를 타고 가족 여행을 다녀오셨습니다.
...
SAVE_SYSTEM_PROMPT="""
# Role
You are a writer who summarizes grandmother's conversation records clearly and concisely.
Guidelines
...
Summarize in one sentence.
Summarize in one sentence.
Write according to the 5W1H principles (Who, When, Where, What, How, Why).
- 예시: 박순자는 외로움을 달래기 위해 매일 아침 집 근처 공원을 산책한다.
- 예시: 박순자는 가족 모임자리에서 복지관 댄스교실 발표회 영상을 자녀들에게 자랑스럽게 공유했다.
"""
#################### 추가 ####################
...
class TritonPythonModel:
    ...
    #################### 추가 ####################
    def _generate_prompt(self, session_id: str, op_type: str = "", system_prompt: str = "", 
...
"""
    ...
    #################### 추가 ####################
    def _get_conversation_history(self, session_id: str, op_type: str) -> str:
        """대화 기록 조회 - op_type에 따라 다르게 처리"""
        # save는 전체 대화 기록 필요
        count = self.client.llen(session_id)
        if count < 10:  # 대화 기록이 10개 미만이면 빈 문자열 반환
            return ""
        messages = self.client.lrange(session_id, -20, -1)  # 전체 대화
    ...
    #################### 추가 ####################
    def _action_handler(self, session_id: str, action: str, query: str) -> str:
        """Build query based on action type and handle query rewriting for chat actions."""
        # 공통 데이터
        user_profile = self._get_user_profile(session_id)
        conversation_history = self._get_conversation_history(session_id, action)
        ...
        if action == "rewrite":
            return self._generate_prompt(
                ...
                user_query=(
                    f"Analyze the key phrases in the following sentence and, referencing personal information, craft a single sentence that recalls memories or experiences related to the keywords. {query}"
                )
            )
        if action == "clear":
            return self._generate_prompt(
        ...
        if action == "save":
            if not conversation_history:
                return self._generate_prompt(session_id="-1", op_type="save")
            return self._generate_prompt(
                session_id=session_id,
                op_type=action,
    ...
    def execute(self, requests):
        ...
        for idx, request in enumerate(requests):
            ...
            match = re.match(r'\((\d+)\)\s*\[(.*?)\](.*)', query_text)
            if not match:
                logger.log_info(f"잘못된 QUERY 텍스트 형식: {query_text}")
                responses.append(pb_utils.InferenceResponse(output_tensors=[]))
                continue
            session_id = match.group(1) if match.group(1) else "-1"
            action = match.group(2).strip() if match.group(2) else "unknown"
            query = match.group(3).strip() if match.group(3) else ""
            # print(session_id, action, query)
            query_augmented = self._action_handler(
                session_id=session_id,
                action=action,
                query=query
            )
